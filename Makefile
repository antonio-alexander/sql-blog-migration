...
goose-status: ## get goose migration status
	@docker ${docker_args} exec -it mysql goose mysql "root:${mysql_root_password}@tcp(localhost:3306)/sql_blog_migration?parseTime=true&&multiStatements=true" --table employees_goose_db_version status

goose-up: ## execute goose up
	@docker ${docker_args} exec -it mysql goose mysql "root:${mysql_root_password}@tcp(localhost:3306)/sql_blog_migration?parseTime=true&&multiStatements=true" --table employees_goose_db_version up

goose-up-by-one: ## execute goose up-by-one
	@docker ${docker_args} exec -it mysql goose mysql "root:${mysql_root_password}@tcp(localhost:3306)/sql_blog_migration?parseTime=true&&multiStatements=true" --table employees_goose_db_version up-by-one
